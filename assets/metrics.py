...
def _init_graphs(num_labels):
    def map_labels_func(y_true):
        ...
        return _map_labels(y_true, num_labels, tf.constant(t0, dtype=tf.float32), tf.constant(t1, dtype=tf.float32))

    def map_predictions_func(y_pred):
        return _map_predictions(y_pred, num_labels)

    # Create graph functions:
    map_labels = tf.function(
        func=map_labels_func,
        input_signature=[tf.TensorSpec(shape=None, dtype=tf.float32)]  # Define the input signature
    )

    map_predictions = tf.function(
        func=map_predictions_func,
        input_signature=[tf.TensorSpec(shape=None, dtype=tf.float32)]  # Define the input signature
    )
